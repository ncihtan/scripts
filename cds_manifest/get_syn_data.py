...
def generate_json(include_non_public_images, include_non_public_htapp_folders):
    ...
    config_path = os.path.join(script_dir, 'config-htan.yml')
    schematic.CONFIG.load_config(config_path)
    ...
    htapp_release_path = os.path.join(script_dir, 'htapp_release1.tsv')
    htapp_release1_folder_names = set(pd.read_csv(htapp_release_path,sep='\t')['Folder Name'])
    ...
    for project_id, dataset_group in metadata_manifests:
        ...
        for dataset in datasets:
            manifest_location = os.path.join(script_dir, "tmp", center_id)
            manifest_path = os.path.join(manifest_location, "synapse_storage_manifest.csv")
            os.makedirs(manifest_location, exist_ok=True)
            ...
            print(component)
            if schema_info["displayName"] in gdc_data_types:
                gdc_data_synapse_ids.extend(manifest_df.entityId)
    ...
    output_path = os.path.join(script_dir, "cds_manifest.synids.txt")
    with open(output_path, "w") as m_f:
        m_f.write("\n".join(gdc_data_synapse_ids) + "\n")
